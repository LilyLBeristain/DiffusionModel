...
def sample_images(args):
    ...
    # Cargar checkpoint — intentar usar EMA si está disponible
    checkpoint = torch.load(args.ckpt_path, map_location=device)
    if "ema_network" in checkpoint:
        print("Using EMA weights for sampling (better quality)")
        network.load_state_dict(checkpoint["ema_network"])
    elif "network_state" in checkpoint:
        network.load_state_dict(checkpoint["network_state"])
    elif "network" in checkpoint:
        network.load_state_dict(checkpoint["network"])
    else:
        raise ValueError("No recognizable network weights found in checkpoint")
...
def main():
    ...
    sample_parser.add_argument("--ckpt_path", type=str, default="results/last.ckpt")
    ...
    if args.command == "sample":
        ...
        fid_value = calculate_fid([args.save_path, args.gt_path], device=args.device)
        print("FID:", fid_value)
