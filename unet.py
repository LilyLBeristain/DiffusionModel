...
class Res(nn.Module):
    def __init__(self, in_channels, out_channels, time_dim, dropout, use_attn=False):
        ...
        self.apply(lambda m: init_w(m) if isinstance(m, (nn.Conv2d, nn.Linear)) else None)
    ...
    def forward(self, inp, time_emb):
        ...
        h = self.dropout(h)
        h = self.conv2(h)
        # FIX: standard residual connection — no division by sqrt(2)
...
class TimeEmbed(nn.Module):
    ...
    def sin_emb(time_input, dim, max_period=10000):
        ...
        args  = time_input[:, None].float() * freqs[None, :] # [B, half_dim]
        # FIX: interleave cos/sin per frequency as per assignment spec:
        # [cos(w1), sin(w1), cos(w2), sin(w2), ...]
        emb = torch.stack([torch.cos(args), torch.sin(args)], dim=-1)
        return emb.reshape(time_input.shape[0], dim)

    def forward(self, time_input):
        return self.mlp(self.sin_emb(time_input, self.freq, self.max_period))
...
class UNet(nn.Module):
    def __init__(
        ...
        self.mid_blocks = nn.ModuleList([
            Res(current_channels, current_channels, time_dim, dropout, True),
        ...
        if self.use_cfg:
            ...
            if cls is None:
                ...
                if scale != 1.0:
                    ...
                    return self._forward(inp, time_uncond) + scale * (
                        self._forward(inp, time_cond) - self._forward(inp, time_uncond)
                    )
                else:
                    cond_emb = self.cls_emb(cls)
                    time_emb = base_time_emb + cond_emb
